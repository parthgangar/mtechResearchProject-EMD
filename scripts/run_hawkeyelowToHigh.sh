...
run_command() {
    ...
    REDIRECT="$currentDir/run_hawkeyelowToHigh/$filename.txt"
    TOPREDIRECT="$currentDir/run_hawkeyelowToHigh/$filename-top.txt"
    for nr_to_free in 1000000 2000000 3000000 4000000 5000000 6000000 7000000 8000000; do
    ...
    for i in 1; do
        ...
        sleep 2
	    funcKhugeDefault
        sudo rmmod remove_lowToHigh
        echo "Loading kernel module"
        sudo insmod /home/gulshan/parth/misc-collection-master/HawkEye/modules/origBloat4.3/remove_hawkeye/remove_lowToHigh.ko pid=$BENCHMARK_PID nr_to_free=$nr_to_free >> $REDIRECT &
        SECONDS=0
...
if [ ! -d "$currentDir/run_hawkeyelowToHigh" ]; then
    mkdir -p $currentDir/run_hawkeyelowToHigh
fi
