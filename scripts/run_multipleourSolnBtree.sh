...
run_command1() {
    ...
    REDIRECT="$currentDir/run_multipleourSoln/$filename-$node_number.txt"
    TOPREDIRECT="$currentDir/run_multipleourSoln/$filename-$node_number-top.txt"
...
if [ ! -d "$currentDir/run_multipleourSoln" ]; then
    mkdir -p $currentDir/run_multipleourSoln
fi


for filename in "${my_list[@]}"; do
    # for nr_to_free in 10000000 15000000 20000000 25000000; do
    for nr_to_free in 6250000; do
    # for nr_to_free in 25000000; do
        rm /tmp/alloctest-bench* &>/dev/null
        rm $currentDir/myfile* &>/dev/null
        echo "Running the experiment with $filename"
        funcKhugeAggr
        sudo rmmod remove_multipleourSoln
        ...
        sudo insmod /home/gulshan/parth/misc-collection-master/HawkEye/modules/origBloat4.3/remove/remove_multipleourSoln.ko pid=$integer_value pid1=$integer_value1 nr_to_free=$nr_to_free
        # sudo insmod /home/gulshan/parth/misc-collection-master/HawkEye/modules/origBloat4.3/remove/remove_multipleourSoln.ko pid=$integer_value pid1=$integer_value1 pid2=$integer_value2 pid3=$integer_value3 nr_to_free=$nr_to_free
