...
run_command1() {
    ...
    REDIRECT="$currentDir/run_multipleourSolnHetero/$filename-$node_number.txt"
    TOPREDIRECT="$currentDir/run_multipleourSolnHetero/$filename-$node_number-top.txt"
...
if [ ! -d "$currentDir/run_multipleourSolnHetero" ]; then
    mkdir -p $currentDir/run_multipleourSolnHetero
fi
    ...
    for nr_to_free in 5000000 7500000 10000000 12500000; do
        ...
        sudo insmod /home/gulshan/parth/misc-collection-master/HawkEye/modules/origBloat4.3/remove/remove_multipleourSoln.ko pid=$integer_value1 pid1=$integer_value nr_to_free=$nr_to_free
        sleep 5
        funcKhugeDefault
