...
run_command() {
    ...
    REDIRECT="$currentDir/run_ourSoln/$filename.txt"
    TOPREDIRECT="$currentDir/run_ourSoln/$filename-top.txt"
    for nr_to_free in 250000 500000 750000 1000000 ; do
    ...
    for i in 1; do
	    ...
	    funcKhugeDefault
        sudo rmmod remove_ourSoln
        echo "Loading kernel module"
        sudo insmod /home/gulshan/parth/misc-collection-master/HawkEye/modules/origBloat4.3/remove/remove_ourSoln.ko pid=$BENCHMARK_PID nr_to_free=$nr_to_free >> $REDIRECT &
        SECONDS=0
...
if [ ! -d "$currentDir/run_ourSoln" ]; then
    mkdir -p $currentDir/run_ourSoln
fi
