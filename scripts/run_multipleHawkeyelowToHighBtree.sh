...
run_command1() {
    ...
    REDIRECT="$currentDir/run_multipleHawkeyelowToHigh/$filename-$node_number.txt"
    TOPREDIRECT="$currentDir/run_multipleHawkeyelowToHigh/$filename-$node_number-top.txt"
...
if [ ! -d "$currentDir/run_multipleHawkeyelowToHigh" ]; then
    mkdir -p $currentDir/run_multipleHawkeyelowToHigh
fi


for filename in "${my_list[@]}"; do
    for nr_to_free in 5000000 10000000 15000000 20000000 25000000 30000000; do
        rm /tmp/alloctest-bench* &>/dev/null
        rm $currentDir/myfile* &>/dev/null
        echo "Running the experiment with $filename"
        funcKhugeAggr
        sudo rmmod remove_multipleHawkeyelowToHigh
        ...
        sudo insmod /home/gulshan/parth/misc-collection-master/HawkEye/modules/origBloat4.3/remove_multipleHawkeye/remove_multipleHawkeyelowToHigh.ko pid=$integer_value pid1=$integer_value1 pid2=$integer_value2 pid3=$integer_value3 nr_to_free=$nr_to_free
